...
/* BEGIN controllers **************** */
// function replaceObjectId(memberId) {
//     return {
...
const teams_get_all = ((req, res) => {
    ...
    .then(async (team) => { 
        if (team !== null) {
            // console.log("Teams Get All team:", team)
            // const members = team.map(team => ({
            //     ...team,
            //     members: team.members.map(memberId => replaceObjectId(memberId))  
            // }));

            const response = await fetch(`http://localhost:${process.env.EMP}/employees/filled`, { 
                method: 'POST',
            ...
            const teams_employees = await response.json();

            console.log("Fetched teams: ", JSON.stringify(teams_employees));
            console.log(`All teams were retrieved from the collection.`);
            res.status(200).json(teams_employees);
        } else {
